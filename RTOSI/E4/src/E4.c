...
int main( void )
{
   ...
   for(int i = 0; i < TOTAL_TECLAS; i++){
      ...
      if(buttonStruct[i].xQueue == NULL){
         printf(ERROR_QUEUE, i);
         while(TRUE);
      };
      buttonStruct[i].xMutex = xSemaphoreCreateMutex();
      if(buttonStruct[i].xMutex == NULL){         
         printf(ERROR_MUTEX, i);
         while(TRUE);
      };
   }
   ...
   for(int i = 0; i < TOTAL_TECLAS; i++){
      xErrorHandler = xTaskCreate(
         led_task_1Hz,                  // Function that implements the task.
         (const char *)"led_task_1Hz",  // Text name for the task.
         configMINIMAL_STACK_SIZE,   // Stack size in words, not bytes.
         &buttonStruct[i],             // Parameter passed into the task.
         tskIDLE_PRIORITY+1,           // Priority at which the task is created.
         task_handles[i]               // Pointer to the task created in the system
